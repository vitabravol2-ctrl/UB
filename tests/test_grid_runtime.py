...
def test_micro_grid_levels_and_budget_split() -> None:
    rt = GridRuntime()
    s = SettingsData(micro_grid_enabled=True, micro_grid_size_ticks=400, micro_grid_step_ticks=10, micro_grid_budget_u=5000.0)
    ...
    assert levels[0].budget_u == 125.0


def test_micro_grid_level_lifecycle_and_telemetry() -> None:
    rt = GridRuntime()
    s = SettingsData(micro_grid_enabled=True, micro_grid_size_ticks=20, micro_grid_step_ticks=10, micro_grid_budget_u=100.0)
    ...
    assert t["GRID BUDGET USED"] == 50.0
    rt.recycle_level(1)
